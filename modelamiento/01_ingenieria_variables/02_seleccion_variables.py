...
def run_feature_selection(input_path: Path = INPUT_PATH,
    ...
    OJO: `output_dir` por defecto es la ruta COMPARTIDA que
    04_ingenieria_variables_produccion.py lee en cada inferencia
    (FEATURES_ESPERADAS = selected_features.csv). Correr esto con los
    defaults sobre un dataset que cambió de tamaño/distribución selecciona
    un set de features distinto al que espera el modelo desplegado
    (`version_modelo.json` -> versiones/{version}/parametros_produccion.json)
    - la próxima predicción fallaría (o, sin el guard de 05_prediccion.py,
    prediciría sobre columnas rellenadas con 0). Para explorar sin ese
    riesgo, pasá un `output_dir` propio; para promover un cambio real,
    hacelo como parte de un reentrenamiento completo
    (entrenamiento/01_entrenar_modelo_produccion.py), no como paso suelto.
    """
    output_dir.mkdir(parents=True, exist_ok=True)
...
def _advertir_si_pisa_artefacto_desplegado(output_dir: Path) -> None:
    """Si `output_dir` es la ruta compartida y hay un modelo desplegado en
    producción, avisa en qué consiste el riesgo antes de sobrescribir (ver
    docstring de run_feature_selection). No bloquea: hay usos legítimos de
    correr esto con los defaults como parte de un reentrenamiento real."""
    if output_dir != OUTPUT_DIR:
        return

    control_path = SCRIPT_DIR.parent.parent / "produccion" / "01_modelo_produccion" \
        / "entrenamiento" / "version_modelo.json"
    if not control_path.exists():
        return

    version_actual = json.loads(control_path.read_text(encoding="utf-8"))["version_actual"]
    print(f"\n⚠  output_dir es la ruta compartida ({OUTPUT_DIR}) y hay un modelo desplegado "
          f"({version_actual}). Si el set de features que resulte de esta corrida difiere del "
          f"que ese modelo espera, la próxima inferencia va a fallar (o, en el peor caso sin el "
          f"guard de 05_prediccion.py, predecir sobre columnas rellenadas con 0) hasta que se "
          f"reentrene con este set nuevo. Ctrl+C ahora si solo estabas explorando.\n")


if __name__ == "__main__":
    _advertir_si_pisa_artefacto_desplegado(OUTPUT_DIR)
